...
def make_lookup_table(name: str, list_name: str, unit_type: str, values: list, precision: int = 2, rated_value = None):
    fields = [
        IDFField(name, "Name"),
        IDFField(list_name, "Independent Variable List Name"),
        IDFField("None" if rated_value is not None else "AutomaticWithDivisor", "Normalization Method"),
        IDFField(1., "Normalization Divisor"),
        ...
        IDFField("", "External File Column Number"),
        IDFField("", "External File Starting Row Number")
    ] + [IDFField(value if rated_value is None else value/rated_value, f"Output Value {i + 1}", precision) for i, value in enumerate(values)]
...
def write_idf(unit: DXUnit,
    ...
    if system_type == EnergyPlusSystemType.ZONEHVAC_PTHP:
        objects.append(("ZoneHVAC:PackagedTerminalHeatPump",[
            ...
            IDFField(0.,"Cooling Outdoor Air Flow Rate {m3/s}"),
            IDFField(0.,"Heating Outdoor Air Flow Rate {m3/s}"),
            IDFField(0.,"No Load Outdoor Air Flow Rate {m3/s}"),
            ...
            IDFField(f"{system_name}Supp Heating Coil","Supplemental Heating Coil Name"),
            IDFField(f"Autosize","Maximum Supply Air Temperature from Supplemental Heater {C}"),
            IDFField(f"","Maximum Outdoor Dry-Bulb Temperature for Supplemental Heater Operation {C}"),
            IDFField(f"DrawThrough","Fan Placement"),
            IDFField(f"{system_name}Fan Mode Schedule","Supply Air Fan Operating Mode Schedule Name"),
        ]))
        objects.append(("Coil:Heating:Electric",[
            IDFField(f"{system_name}Supp Heating Coil", "Name"),
            IDFField(f"{system_name}Schedule","Availability Schedule Name"),
            IDFField(1., "Efficiency"),
            IDFField(0., "Nominal Capacity"),
            IDFField(f"{system_name}Supply Fan Outlet Node","Air Inlet Node Name"),
            IDFField(f"{system_name}Unitary Outlet Node","Air Outlet Node Name"),
        ]))
    elif system_type == EnergyPlusSystemType.UNITARY_SYSTEM:
        objects.append(("AirLoopHVAC:UnitarySystem",[
            IDFField(f"{system_name}Unitary", "Name"),
            IDFField(f"Load", "Control Type"),
            IDFField(f"","Controlling Zone or Thermostat Location"),
            ...
            IDFField(f"Coil:Heating:DX:VariableSpeed","Heating Coil Object Type"),
            IDFField(f"{system_name}Heating Coil","Heating Coil Name"),
            IDFField(unit.gross_integrated_heating_capacity()/unit.gross_total_cooling_capacity(),"DX Heating Coil Sizing Ratio"),
            IDFField(f"Coil:Cooling:DX:VariableSpeed","Cooling Coil Object Type"),
            IDFField(f"{system_name}Cooling Coil","Cooling Coil Name"),
            ...
            IDFField(f"","Cooling Fraction of Autosized Cooling Supply Air Flow Rate"),
            IDFField(f"","Cooling Supply Air Flow Rate Per Unit of Capacity {m3/s-W}"),
            ...
            IDFField(f"","Heating Fraction of Autosized Heating Supply Air Flow Rate"),
            IDFField(f"","Heating Supply Air Flow Rate Per Unit of Capacity {m3/s-W}"),
            ...
            IDFField(f"","No Load Fraction of Autosized Cooling Supply Air Flow Rate"),
            IDFField(f"","No Load Fraction of Autosized Heating Supply Air Flow Rate"),
            IDFField(f"","No Load Supply Air Flow Rate Per Unit of Capacity During Cooling Operation {m3/s-W}"),
            IDFField(f"","No Load Supply Air Flow Rate Per Unit of Capacity During Heating Operation {m3/s-W}"),
            IDFField(f"Autosize","Maximum Supply Air Temperature {C}"),
            IDFField(f"","Maximum Outdoor Dry-Bulb Temperature for Supplemental Heater Operation {C}"),
            IDFField(f"","Outdoor Dry-Bulb Temperature Sensor Node Name"),
            IDFField(f"","Maximum Cycling Rate {cycles/hr}"),
            IDFField(f"","Heat Pump Time Constant {s}"),
            IDFField(f"","Fraction of On-Cycle Power Use"),
            IDFField(f"","Heat Pump Fan Delay Time {s}"),
            IDFField(f"","Ancilliary On-Cycle Electric Power {W}"),
            IDFField(f"","Ancilliary Off-Cycle Electric Power {W}"),
            IDFField(f"","Design Heat Recovery Water Flow Rate {m3/s}"),
    ...
    objects.append(("Schedule:Compact",[
        IDFField(f"{system_name}Schedule", "Name"),
        IDFField(f"Binary Control", "Schedule Type Limits Name"),
        IDFField(f"Through: 12/31", ""),
        IDFField(f"For: AllDays", ""),
        IDFField(f"Until: 24:00", ""),
        IDFField(1, "Value"),
    ]))

    objects.append(("Schedule:Compact",[
        IDFField(f"{system_name}Fan Mode Schedule", "Name"),
        IDFField(f"Fan Mode Control", "Schedule Type Limits Name"),
        IDFField(f"Through: 12/31", ""),
        IDFField(f"For: AllDays", ""),
        IDFField(f"Until: 24:00", ""),
        IDFField(0, "Value"),
    ...
    fan_fields = [
        ...
        IDFField(f"{system_name}Unitary Outlet Node" if system_type == EnergyPlusSystemType.UNITARY_SYSTEM else f"{system_name}Supply Fan Outlet Node","Air Outlet Node Name"),
        IDFField("Autosize" if autosize else unit.fan.airflow(max_fan_speed),"Design Maximum Air Flow Rate"),
        IDFField(f"Discrete","Speed Control Method"),
        IDFField(unit.fan.airflow_ratio(min_fan_speed,max_fan_speed),"Electric Power Minimum Flow Rate Fraction"),
        IDFField(max_fan_speed_pressure,"Design Pressure Rise"),
        IDFField(1.,"Motor Efficiency"), # TODO: Replace if motor ever separated from fan
        IDFField(1.,"Motor In Air Stream Fraction"),
        IDFField("Autosize" if autosize else unit.fan.power(max_fan_speed),"Design Electric Power Consumption"),
        IDFField(f"TotalEfficiencyAndPressure","Design Power Sizing Method"),
        IDFField(f"","Electric Power Per Unit Flow Rate"),
        IDFField(f"","Electric Power Per Unit Flow Rate Per Unit Pressure"),
        IDFField(unit.fan.efficiency(max_fan_speed),"Fan Total Efficiency"),
        IDFField(f"","Electric Power Function of Flow Fraction Curve Name"),
        IDFField(max_fan_speed_pressure,"Night Ventilation Mode Pressure Rise"),
        IDFField(f"","Night Ventilation Mode Flow Fraction"),
        IDFField(f"","Motor Loss Zone Name"),
        IDFField(f"","Motor Loss Radiative Fraction"),
    ...
    for i, speed in enumerate(fan_speed_order_map):
        ep_speed = i + 1
        fan_speed = [
            IDFField(unit.fan.airflow_ratio(speed, max_fan_speed), f"Speed {ep_speed} Flow Fraction"),
            IDFField(unit.fan.power_ratio(speed, max_fan_speed), f"Speed {ep_speed} Electric Power Fraction"),
        ]
    ...
    objects.append(("Table:IndependentVariableList", [
        ...
        IDFField(f"{system_name}Cooling Indoor Wetbulb", "Independent Variable 1 Name"),
        IDFField(f"{system_name}Cooling Outdoor Drybulb", "Independent Variable 2 Name"),
    ])),

    objects.append(make_independent_variable(
        f"{system_name}Coil Flow Fraction",
        "Dimensionless",
        1.,
        flow_fractions
    ))
    ...
    objects.append(("Table:IndependentVariableList", [
        ...
        IDFField(f"{system_name}Heating Indoor Drybulb", "Independent Variable 1 Name"),
        IDFField(f"{system_name}Heating Outdoor Drybulb", "Independent Variable 2 Name"),
    ])),
    ...
    cooling_coil = [
        ...
        IDFField("Autosize" if autosize else unit.gross_total_cooling_capacity(), "Gross Rated Total Cooling Capacity at Selected Nominal Speed Level", 2),
        IDFField("Autosize" if autosize else unit.A_full_cond.rated_volumetric_airflow, "Rated Air Flow Rate at Selected Nominal Speed Level", 2),
    ...
    for speed in reversed(range(unit.number_of_cooling_speeds)):
        ...
        cooling_speed = [
          IDFField(rated_capacity, f"Speed {ep_speed} Reference Unit Gross Rated Total Cooling Capacity", 1),
        ...
        objects.append(make_lookup_table(
            ...
            eirs,
            4,
            rated_value=1./rated_cop if normalize else None
        ))
    ...
    heating_coil = [
        ...
        IDFField("Autosize" if autosize else unit.gross_steady_state_heating_capacity(), "Gross Rated Heating Capacity at Selected Nominal Speed Level", 2),
        IDFField("Autosize" if autosize else unit.H1_full_cond.rated_volumetric_airflow, "Rated Air Flow Rate at Selected Nominal Speed Level", 2),
    ...
    objects.append(("Curve:Biquadratic", [
        IDFField(f"{system_name}Defrost EIR", "Name"),
        IDFField(1./NRELDXModel.get_cooling_cop60(unit.model), "Coefficient1 Constant", 4),
        IDFField(0., "Coefficient2 x"),
        IDFField(0., "Coefficient3 x**2"),
        IDFField(0., "Coefficient4 y"),
        IDFField(0., "Coefficient5 y**2"),
        IDFField(0., "Coefficient6 x*y"),
        IDFField(koozie.convert(50.,"°F","°C"), "Minimum Value of x"),
        IDFField(koozie.convert(80.,"°F","°C"), "Maximum Value of x"),
        IDFField(koozie.to_u(unit.heating_off_temperature,"°C"), "Minimum Value of y"),
        IDFField(koozie.to_u(unit.defrost.high_temperature,"°C"), "Maximum Value of y"),
    ]))
    ...
    for speed in reversed(range(unit.number_of_heating_speeds)):
        ...
        heating_speed = [
          IDFField(rated_capacity, f"Speed {ep_speed} Reference Unit Gross Rated Heating Capacity", 2),
