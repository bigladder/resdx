...
class FanConditions:
  def __init__(self, external_static_pressure=fr_u(0.15, "inch_H2O_39F"),
                     speed_setting=0):
...
class Fan:
  ...
  def efficacy(self, conditions):
    min_esp_2017 = fr_u(0.2, "inch_H2O_39F")
    min_esp_2021 = fr_u(0.50, "inch_H2O_39F")
    weight = (conditions.external_static_pressure - min_esp_2017) / (min_esp_2021 - min_esp_2017)
    weight = max(min(1., weight),0.)
    return fr_u(0.365,'W/(cu_ft/min)')*(1. - weight) + fr_u(0.441,'W/(cu_ft/min)')*weight
...
class PSCFan(Fan):
  ...
  def __init__(
    self,
    airflow_rated,
    external_static_pressure_rated=fr_u(0.15, "inch_H2O_39F"),
    efficacy_rated=fr_u(0.365,'W/(cu_ft/min)')):
      super().__init__(airflow_rated, external_static_pressure_rated, efficacy_rated)
      self.AIRFLOW_COEFFICIENT = fr_u(10.,'cu_ft/min')
      self.AIRFLOW_EXP_COEFFICIENT = fr_u(5.355391179,'1/inch_H2O_39F')
