...
for seer in seer_range:
    root_fn = (
        lambda x: resdx.DXUnit(rated_gross_cooling_cop=x, input_seer=seer).seer() - seer
    )
...
for hspf in hspf_range:
    root_fn = (
        lambda x: resdx.DXUnit(rated_gross_heating_cop=x, input_hspf=hspf).hspf() - hspf
    )
