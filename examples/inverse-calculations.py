...
def plot(x, y, xlabel, ylabel):
    ...
    plt.xlabel(xlabel)
    plt.ylabel(ylabel)
    plt.savefig(f'output/{ylabel.replace(" ", "")}_vs_{xlabel.replace(" ", "")}.png')
...
plot(seer_range, cops_from_seer, "SEER", "Gross (fan not included) COP (at A conditions)")
...
plot(hspf_range, cops_from_hspf, "HSPF", "Gross (fan not included) COP (at H1 conditions)")
