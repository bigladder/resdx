...
for seer in seer_range:
    root_fn = lambda x : resdx.DXUnit(gross_cooling_cop_rated = [x], fan_efficacy_cooling_rated = [resdx.fan_efficacy(seer)], c_d_cooling=resdx.c_d(seer)).seer() - seer
...
for seer in seer_range:
    root_fn = lambda x : resdx.DXUnit(
        number_of_input_stages = 2,
        gross_cooling_cop_rated = x,
        fan_efficacy_cooling_rated = resdx.fan_efficacy(seer),
        c_d_cooling=resdx.c_d(seer)
    ).seer() - seer
    cop, solution = optimize.newton(root_fn,seer/3.33, full_output = True)
    #print(f"seer = {seer:.1f}, cop = {cop:.2f}, fan_eff = {resdx.to_u(fan_efficacy(seer),'W/(cu_ft/min)'):.3f}, C_d = {c_d(seer):.3f}, converged = {solution.converged}, iter = {solution.iterations}")
    cops_from_seer.append(cop)
...
for hspf in hspf_range:
    root_fn = lambda x : resdx.DXUnit(gross_heating_cop_rated = [x], fan_efficacy_heating_rated = [resdx.fan_efficacy(resdx.estimated_seer(hspf))], c_d_heating=resdx.c_d(resdx.estimated_seer(hspf))).hspf() - hspf
...
for hspf in hspf_range:
    root_fn = lambda x : resdx.DXUnit(
        number_of_input_stages = 2,
        gross_heating_cop_rated = x,
        fan_efficacy_heating_rated = resdx.fan_efficacy(resdx.estimated_seer(hspf)),
        c_d_heating=resdx.c_d(resdx.estimated_seer(hspf))
    ).hspf() - hspf
    cop, solution = optimize.newton(root_fn,hspf/2.0, full_output = True)
    #print(f"hspf = {hspf:.1f}, cop = {cop:.2f}, fan_eff = {resdx.to_u(fan_efficacy(estimated_seer(hspf)),'W/(cu_ft/min)'):.3f}, C_d = {c_d(estimated_seer(hspf)):.3f},converged = {solution.converged}, iter = {solution.iterations}")
    cops_from_hspf.append(cop)
