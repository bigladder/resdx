...
seer2 = 14.3
hspf2 = 7.5
cop_c, solution_c = optimize.newton(lambda x : resdx.DXUnit(number_of_cooling_speeds=2, rated_gross_cooling_cop=x, input_seer=seer2, rating_standard=resdx.AHRIVersion.AHRI_210_240_2023).seer() - seer2, seer2/3.33, full_output = True)
cop_h, solution_h = optimize.newton(lambda x : resdx.DXUnit(number_of_heating_speeds=2, rated_gross_heating_cop=x, input_hspf=hspf2, rating_standard=resdx.AHRIVersion.AHRI_210_240_2023).hspf() - hspf2, hspf2/2., full_output = True)
dx_unit = resdx.DXUnit(number_of_cooling_speeds=2, rated_gross_cooling_cop=cop_c, rated_gross_heating_cop=cop_h, input_seer=seer2, input_hspf=hspf2)
...
with open(f"{output_directory_path}/{file_name}-motor.RS0005.json", "w") as file:
  json.dump(representation["performance"]["indoor_fan_representation"]["performance"]["motor_representation"], file, indent=4)
